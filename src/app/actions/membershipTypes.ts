...
export async function createMembershipType(formData: FormData) {
    ...
    const daysValid = formData.get('daysValid') ? parseInt(formData.get('daysValid') as string) : null
    const entries = formData.get('entries') ? parseInt(formData.get('entries') as string) : null
