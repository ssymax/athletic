                    ...
                    {products.map(product => (
                        ...
                        >
                            <div>
                                ...
                                <p className="text-xs text-muted font-medium uppercase tracking-tight">{product.category}</p>
                            </div>
                            <div className="flex justify-between items-end mt-4">
                                <span className="price-main">{product.price.toFixed(2)} <small>PLN</small></span>
                                <span className={`stock-pill ${product.stock < 10 ? "stock-pill--low" : ""}`}>
                                    Stan: {product.stock}
                                </span>
                            </div>
                        </button>
            ...
            <div className="card pos-cart">
                            ...
                            {cart.map(item => (
                                <div key={item.id} className="cart-item">
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-muted">{item.price.toFixed(2)} zł x {item.quantity}</p>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="quantity-control">
                                            <button onClick={() => updateQuantity(item.id, -1)} className="btn btn-ghost btn-sm">-</button>
                ...
                <div className="section-divider">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <span className="total-value">{total.toFixed(2)} <small>PLN</small></span>
                    </div>
